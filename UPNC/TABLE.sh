...
workdir='/home/qcapdet/M2BI/Projet_long/Project_PUs/UPNC'
list_file="$workdir/UNIQ_SORT.txt"
#list_file="$workdir/Test.txt"
dir_PDBChains="$workdir/PDBChains"

# Création d'un fichier TSV
echo -e "ID_PDB_PU\tPos_PU_ori\tPos_PU_struc_PDB\tPos_PU_seq_PDB\tPos_PU_UNP\tID_UNP" > $workdir/TABLE_UPNC.tsv
...
for file_name in $(cat "$list_file"); do
    ...
    if [[ -f "$tsv_file" ]]; then
        ...
        echo -e "${file_name}\t${residue_start}_${residue_end}\t${Start_PU_struc_f}_${End_PU_struc_f}\t${Start_PU_seq}_${End_PU_seq}\t${Start_PU_UNP_f}_${End_PU_UNP_f}\t${ID_UNP}" >> $workdir/TABLE_UPNC.tsv
